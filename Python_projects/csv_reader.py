...
sites_changed = 0
# Define update cert log function
...
# Define update countries function
...
with open("test.csv", 'r') as file:
    ...
    for l in reader:
        site = (l["Publisher"], l["Folder"], l["Country"])
        ...
        update_creative_site = "C:/Users/MikkelAndersen/Desktop/Repos/adnami-publishers/src/publishers/" + cs + "/config.js"
        update_cert_log = "C:/Users/MikkelAndersen/Desktop/Repos/adnami-publishers/src/publishers/" + cs + "/cert_log.csv"
            ...
            if exists(update_creative_site) and publisher !=  '':
                with open(update_creative_site) as x:
                    if publisher_defined in x.read():
                        ...
                        sites_changed += 1
                        print(f"Found existing publisher on {cs}, replacing now")
                    else:
                        publisher_search = "  publisher: ['" + str(publisher) + "'],\n }"
                        f = open(update_creative_site,'r')
                        filedata = f.read()
                        ...
                        update_log(update_cert_log)
                        sites_changed += 1
                        print(f"No publisher defined on {cs}, adding one now")
            else:
                print(f"Could not find the site {cs} requested")
        except:
            pass
print(sites_changed)
